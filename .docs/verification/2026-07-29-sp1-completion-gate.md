...
| G10 DS非回帰 | PASS | `node scripts/design-audit.js --app=url-encoder` → 1。`git restore --source=HEAD -- .docs/design-audit-result.json` → 0。`git diff -- .docs/design-audit-result.json` → 0 | 期待どおりDS-002「バックリンクがない」1件のみ。MUST違反1件・警告0件。監査による一時変更blobから初期blobへexact復元し、最終差分なし。 |
